...
#if !defined(AI_NETWORKMF_INPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_in_1[AI_NETWORKMF_IN_1_SIZE_BYTES];
ai_i8* data_ins[AI_NETWORKMF_IN_NUM] = {
data_in_1
};
#else
ai_i8* data_ins[AI_NETWORKMF_IN_NUM] = {
...
#if !defined(AI_NETWORKMF_OUTPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_out_1[AI_NETWORKMF_OUT_1_SIZE_BYTES];
ai_i8* data_outs[AI_NETWORKMF_OUT_NUM] = {
data_out_1
};
#else
ai_i8* data_outs[AI_NETWORKMF_OUT_NUM] = {
...
AI_ALIGNED(32)
static uint8_t pool0[AI_NETWORKMF_DATA_ACTIVATION_1_SIZE];

ai_handle data_activations0[] = {pool0};

/* AI objects ----------------------------------------------------------------*/

static ai_handle networkmf = AI_HANDLE_NULL;
...
static int ai_boostrap(ai_handle *act_addr)
{
  ...
  err = ai_networkmf_create_and_init(&networkmf, act_addr, NULL);
  if (err.type != AI_ERROR_NONE) {
    ai_log_err(err, "ai_networkmf_create_and_init");
    return -1;
  }

  ai_input = ai_networkmf_inputs_get(networkmf, NULL);
  ai_output = ai_networkmf_outputs_get(networkmf, NULL);

#if defined(AI_NETWORKMF_INPUTS_IN_ACTIVATIONS)
  /*  In the case where "--allocate-inputs" option is used, memory buffer can be
   *  used from the activations buffer. This is not mandatory.
   */
  for (int idx=0; idx < AI_NETWORKMF_IN_NUM; idx++) {
   ...
   */
  for (int idx=0; idx < AI_NETWORKMF_OUT_NUM; idx++) {
...
static int ai_run(void)
{
  ...
  batch = ai_networkmf_run(networkmf, ai_input, ai_output);
  if (batch != 1) {
    ai_log_err(ai_networkmf_get_error(networkmf),
        "ai_networkmf_run");
    return -1;
  }
